...
- **201 tests passing (45 files)**, typecheck + lint clean, production build on Vercel
- Migrations applied through **068** (month plans + learning goal + letter weeks 3/4)
...
## Pending migrations (apply via SQL editor)

| Migration | Feature                                                                                         | Status  |
| --------- | ----------------------------------------------------------------------------------------------- | ------- |
| 059       | `fortnights.grade` (per-grade plans)                                                            | pending |
| 060       | Format school-sharing                                                                           | pending |
| 062       | `fortnights.unidades_didacticas`                                                                | pending |
| 063       | `students.nee_notes_encrypted`                                                                  | pending |
| 064       | `vocabulary_items.image_url` + `vocab-images` bucket                                            | pending |
| 069       | `game_players` + `game_plays` + `materials.homework_min_correct` + `teachers.share_game_scores` | pending |

After pushing: `supabase gen types typescript --linked > lib/database.types.ts`
