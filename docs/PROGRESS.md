...
- **NEM decisions as optional dropdowns (audit + ejes)**: audited the whole knowledge dir (`context/*.md`, `lib/nem/*`) — metodologías (`MODALIDADES`→`METHODOLOGY_STRUCTURE`, correct), contenidos+PDAs (`CONTENIDOS_FASE2_3`, correct) are already per-unit optional dropdowns; the one gap was **ejes articuladores** (model auto-picked). Added an optional per-unit **"Ejes articuladores"** multi-select (7 canonical from `EJES_ARTICULADORES`) in `nueva/page.tsx` → stored in `unidades_didacticas[].ejes` → injected as `<ejes_seleccionados>` in both `buildQuincenaPrompt` and `buildTallerPrompt` (overrides the grounding's "pick 2-3"; empty = model chooses as before). Drift guard: `grounding.test.ts` asserts `EJES_FASE2` names === `EJES_ARTICULADORES` (the 3 ejes definitions must stay identical). Campos formativos remain implied by the contenidos picker (grouped by campo); PDAs travel verbatim with each contenido. No migration (JSONB). 175 tests.
- **Auto-fill blank NEM dropdowns (rotation-aware, relevance-first)**: when the teacher leaves metodología / ejes / contenidos blank, generation now auto-selects NEM-aligned options that FIT the topic first and only vary from recent plans as a tie-breaker (Alan's choice). `lib/planner/auto-select.ts`: `extractRecentChoices` (pure, from recent plans' `unidades_didacticas`) + `autoSelectNem` (one best-effort Haiku call picking a metodología from `METODOLOGIAS_VALIDAS` and 2-3 ejes from the canonical 7, told which were used recently). `generate-document` fetches the teacher's last 6 plans' choices, then: metodología — a new **"✨ Automático (la IA elige)"** default option on Unit 1's dropdown resolves to a real methodology (fallback Proyecto, never leaves the placeholder); ejes — auto-picked into `unit0.ejes` when none chosen (flows through the existing `<ejes_seleccionados>` block); contenidos — `selectRelevantContenidos` gains an `avoid` param seeded with recently-used contenidos (relevance still wins). All best-effort → any failure keeps the prior behavior. Tests: `auto-select.test.ts` (extraction dedup/placeholder-skip). 178 tests.
- **Theme flash (FOUC) fixed**: the color theme + app font are mirrored into an `app_design` cookie (`lib/design/vars.ts`); a tiny pre-paint `<script>` in the root layout applies the CSS vars before first paint, eliminating the gold→theme flash. Cookie synced on shell load + perfil save (before reload). `/jugar` already SSR-applies the owner theme.
