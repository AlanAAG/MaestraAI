...
- **Schema applied through 058** (per Alan); **migrations 059 (`fortnights.grade`) + 060 (template school-sharing) + 062 (`fortnights.unidades_didacticas`) + 063 (`students.nee_notes_encrypted`) pending push** — per-grade planeaciones, format sharing, teacher-defined units, and per-student NEE notes need them (all degrade gracefully until pushed; 061 = waitlist). After pushing, regen `lib/database.types.ts`.
